...
var mSearch2 = {
    ...
    options: {
        ...
        selected_tpl: '<a href="#" data-id="_id_" class="mse2_selected_link btn-group"><button class="btn btn-primary btn-sm p-1 px-3 disabled">_title_</button><button class="btn btn-primary btn-sm btn-close border-primary p-1">x</button></a>',
        selected_wrapper_tpl: '<strong>_title_:&emsp;</strong>',
    ...
    initialize: function (selector) {
        ...
        $('.reset-current-filter').on('click', function (e) {
            preventDefault();
            let target = e.currentTarget;
            if (target) {
                let filter = target.dataset.filter;
                if (filter) {
                    return mSearch2.resetCurrentFilter(filter);
                }
            }
        });
        ...
        if (this.total.length > 0)
            for (var i = 0; i < this.total.length; i++) {
                ...
                mSearch2.setTotal($(this.total[i]).text());
            }
    ...
    setTotal: function (total) {
        if (this.total.length != 0) {
            if (!total || total == 0) {
                ...
                for (var i = 0; i < this.total.length; i++) {
                    //console.log(this.total[i]);
                    $(this.total[i]).parent().show();
                    $(this.total[i]).text(total);
                }
    ...
    resetCurrentFilter: function (filter) {
        ...
        if (hash.hasOwnProperty(filter)) {
            ...
            if (filterButton[0] && filterButton[0].classList.contains(this.options.active_class)){
                filterButton[0].classList.remove(this.options.active_class)
            }
        }
        return this.submit();
    },
